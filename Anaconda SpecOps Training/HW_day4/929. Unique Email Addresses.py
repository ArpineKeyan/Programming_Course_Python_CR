...
diff_addresses = []
for item in emails:
    email_parts = item.partition("@")                       #getting two parts of email, username and domain separated by "@"
    username_without_dots = ''.join(email_parts[0].split('.'))          #getting username without dots - splittiong by dots and then joining parts
    username_removed_after_plus = username_without_dots.split("+")[0]        #remooving the part after the "+" sign
    new_email = username_removed_after_plus + "@" + email_parts[2]

    if new_email not in diff_addresses:                                  #getting the list of different addresses
        diff_addresses.append(new_email)

print(len(diff_addresses))
